...
var aesKey =  "1!@23#%^87zhqJIY";
var iv  = CryptoJS.enc.Utf8.parse(aesKey);


var aesEncrypt2 = function(data, keyStr) {
...
export function aesDecrypt2(data, key) {
	// debugger;
    var decrypted = CryptoJS.AES.decrypt(data, key, {iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Iso10126});
    return decrypted.toString(); // 16进制字符串
};
function MD5Triple(account,pwd){  // 三次md5
    var key_hash = CryptoJS.MD5(account+"_"+pwd);
...
export  function aesEncrypt(key_hash3,md){
    var encrypted = CryptoJS.AES.encrypt(md, key_hash3, { iv: iv,mode:CryptoJS.mode.CBC,padding:CryptoJS.pad.Iso10126});
...
// 加密资金密码
function md5ForPwd(pwd, account){
	if( !account ){
		var userVal = localStorage.getItem('account');
	}else{
		var userVal = account
	}
	var md5 = userVal+'_'+ pwd.toString();
		// var md5 = userVal+'_'+ CryptoJS.MD5(userVal, 32).toString();
...
function GetQueryString(str, query){
	...
	var rs = new RegExp("(^|)"+str+"=([^\&]*)(\&|$)","gi").exec(query), tmp;
    if(tmp=rs){
        return tmp[2];
    }    
    return "";
